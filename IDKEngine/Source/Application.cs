    ...
    protected override void OnRender(float dT)
    {
        ...
        if (RenderMode_ == RenderMode.Rasterizer)
        {
            ...
            else
            {
                ...
                if (RasterizerPipeline.ActiveVariableRateShadingSource == RasterPipeline.ActiveVrsSource.MotionAware)
                {
                    RasterizerPipeline.MotionAwareVRS.DebugRender(TonemapAndGamma.Result);
                }
                else
                {
