...
class LightingShadingRateClassifier : IDisposable
{
    ...
    public record struct GpuSettings
    {
        ...
        // ▼▼▼ [필수 추가] 쉐이더랑 줄 맞추기용 빈칸 ▼▼▼
        private float _Pad0;              // 4 byte (이게 없으면 데이터가 밀립니다!)
        // ▼▼▼ [여기 추가!] 쉐이더랑 순서, 타입이 똑같아야 합니다. ▼▼▼
        public Vector2 MousePos;  // 마우스 위치
        public int IsFoveated;    // 스위치 (0 or 1)

        public int VrsMode;
        public int UseMotionFusion = 0;

        // Motion + Frequency Map fusion
        public int UseFrequencyMapFusion = 1;

        // Frequency Map 보호 강도
        // 0: 사용 안 함
        // 1: High detail만 보호
        // 2: High + Medium detail 보호
        public int FrequencyProtectLevel = 2;
    ...
    public void Compute(BBG.Texture shaded, BBG.Texture frequencyMap = null)
    {
        BBG.Computing.Compute("Generate Shading Rate Image", () =>
        {
            ...
            {
                BBG.Cmd.BindTextureUnit(frequencyMap, 2);
            }
