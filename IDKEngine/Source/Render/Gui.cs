...
using static IDKEngine.Bvh.PreSplitting;
    ...
    private unsafe void DrawMyGui(Application app)
    {
        ...
        if (ImGui.Begin("Renderer"))
        {
            ...
            if (app.RenderMode_ == Application.RenderMode.Rasterizer)
            {
                ...
                {
                    bool vrsSupported =
                        LightingShadingRateClassifier.IS_SUPPORTED ||
                        MotionAwareVRSClassifier.IS_SUPPORTED;

                    ImGui.Text($"NV_shading_rate_image: {vrsSupported}");
                    ToolTipForItemAboveHovered(
                        "Allows the renderer to choose a unique shading rate on each 16x16 tile\n" +
                        "as a measure of increasing performance by decreasing fragment shader\n" +
                        "invocations in regions where less detail may be required."
                    );

                    if (CheckBoxEnabled("IsVariableRateShading", ref app.RasterizerPipeline.IsVariableRateShading, vrsSupported))
                    {
                        resetPathTracer = true;
                    }
                    ...
                    // 실제 렌더링에 적용할 VRS 소스 선택
                    if (ImGui.BeginCombo("Active VRS Source", app.RasterizerPipeline.ActiveVariableRateShadingSource.ToString()))
                    {
                        RasterPipeline.ActiveVrsSource[] sources = Enum.GetValues<RasterPipeline.ActiveVrsSource>();

                        for (int i = 0; i < sources.Length; i++)
                        {
                            RasterPipeline.ActiveVrsSource source = sources[i];
                            bool isSelected = source == app.RasterizerPipeline.ActiveVariableRateShadingSource;

                            if (ImGui.Selectable(source.ToString(), isSelected))
                            {
                                app.RasterizerPipeline.ActiveVariableRateShadingSource = source;
                                resetPathTracer = true;
                            }
                    ...
                    ToolTipForItemAboveHovered(
                        "Selects which VRS classifier is used for actual rendering.\n" +
                        "LightingVRS and MotionAwareVRS keep their own independent settings."
                    );

                    if (app.RasterizerPipeline.ActiveVariableRateShadingSource == RasterPipeline.ActiveVrsSource.Lighting)
                    {
                        ImGui.TextDisabled("Lighting source uses Original + Motion Fusion + Frequency Map Protection.");
                    }
                    else
                    {
                        ImGui.TextDisabled("MotionAware source uses the separate MotionAwareVRSClassifier.");
                    }
                    ...
                    // LightingVRS 설정
                    if (ImGui.TreeNodeEx("Lighting VRS Settings", ImGuiTreeNodeFlags.DefaultOpen))
                    {
                        if (ImGui.BeginCombo("VRS Mode##LightingVRS", app.RasterizerPipeline.VariableRateShadingMode.ToString()))
                        {
                            ...
                            for (int i = 0; i < modes.Length; i++)
                            {
                                ...
                                // MotionAware는 별도 Active VRS Source로 관리하므로
                                // VrsMode 목록에 남아있다면 GUI에서는 숨깁니다.
                                if (mode.ToString() == "MotionAware")
                                {
                                    continue;
                                }
                        ...
                        if (app.RasterizerPipeline.VariableRateShadingMode == RasterPipeline.VrsMode.Original)
                        {
                            ImGui.TextDisabled("Original mode supports Motion Fusion and Frequency Map Protection.");
                        }
                        else if (app.RasterizerPipeline.VariableRateShadingMode == RasterPipeline.VrsMode.FrequencyMap)
                        {
                            ImGui.TextDisabled("FrequencyMap mode shows standalone Frequency Map VRS.");
                        }
                        else if (app.RasterizerPipeline.VariableRateShadingMode == RasterPipeline.VrsMode.Distance)
                        {
                            ImGui.TextDisabled("Distance mode shows standalone distance-based VRS.");
                        }
                        ...
                        if (ImGui.BeginCombo("DebugMode##LightingVRS", app.RasterizerPipeline.LightingVRS.Settings.DebugValue.ToString()))
                        {
                            LightingShadingRateClassifier.DebugMode[] debugModes =
                                Enum.GetValues<LightingShadingRateClassifier.DebugMode>();
                        ...
                        ImGui.SeparatorText("Base Factors");

                        ImGui.SliderFloat("SpeedFactor##LightingVRS", ref app.RasterizerPipeline.LightingVRS.Settings.SpeedFactor, 0.0f, 1.0f);
                        ImGui.SliderFloat("LumVarianceFactor##LightingVRS", ref app.RasterizerPipeline.LightingVRS.Settings.LumVarianceFactor, 0.0f, 0.3f);

                        ImGui.SeparatorText("Motion + Frequency Fusion");

                        var lightingSettings = app.RasterizerPipeline.LightingVRS.Settings;

                        bool useMotionFusion = lightingSettings.UseMotionFusion == 1;
                        if (ImGui.Checkbox("Use Motion Fusion##LightingVRS", ref useMotionFusion))
                        {
                            lightingSettings.UseMotionFusion = useMotionFusion ? 1 : 0;
                            app.RasterizerPipeline.LightingVRS.Settings = lightingSettings;
                            resetPathTracer = true;
                        }

                        ToolTipForItemAboveHovered(
                            "Uses tile motion to make the shading rate more coarse in moving areas.\n" +
                            "This is applied only in Original VRS mode."
                        );

                        bool useFrequencyMapFusion = lightingSettings.UseFrequencyMapFusion == 1;
                        if (ImGui.Checkbox("Use Frequency Map Protection##LightingVRS", ref useFrequencyMapFusion))
                        {
                            lightingSettings.UseFrequencyMapFusion = useFrequencyMapFusion ? 1 : 0;
                            app.RasterizerPipeline.LightingVRS.Settings = lightingSettings;
                            resetPathTracer = true;
                        }

                        ToolTipForItemAboveHovered(
                            "Protects high-frequency detail areas from becoming too coarse.\n" +
                            "This is applied after Motion Fusion in Original VRS mode."
                        );

                        int frequencyProtectLevel = lightingSettings.FrequencyProtectLevel;
                        if (ImGui.SliderInt("Frequency Protect Level##LightingVRS", ref frequencyProtectLevel, 0, 2))
                        {
                            lightingSettings.FrequencyProtectLevel = frequencyProtectLevel;
                            app.RasterizerPipeline.LightingVRS.Settings = lightingSettings;
                            resetPathTracer = true;
                        }
                        ...
                        ToolTipForItemAboveHovered(
                            "0: Off\n" +
                            "1: Protect high-detail areas only\n" +
                            "2: Protect high and medium-detail areas"
                        );
                    ...
                    // MotionAwareVRS 설정
                    if (ImGui.TreeNodeEx("Motion Aware VRS Settings", ImGuiTreeNodeFlags.DefaultOpen))
                    {
                        var motionSettings = app.RasterizerPipeline.MotionAwareVRS.Settings;

                        if (ImGui.BeginCombo("DebugMode##MotionAwareVRS", motionSettings.DebugValue.ToString()))
                        {
                            MotionAwareVRSClassifier.DebugMode[] debugModes =
                                Enum.GetValues<MotionAwareVRSClassifier.DebugMode>();

                            for (int i = 0; i < debugModes.Length; i++)
                            {
                                MotionAwareVRSClassifier.DebugMode mode = debugModes[i];

                                bool isSelected = mode == motionSettings.DebugValue;

                                if (ImGui.Selectable(mode.ToString(), isSelected))
                                {
                                    motionSettings.DebugValue = mode;
                                }
                        ...
                        ImGui.SeparatorText("Motion Thresholds");

                        ImGui.SliderFloat("MotionLow##MotionAwareVRS", ref motionSettings.MotionLow, 0.0f, 10.0f);
                        ImGui.SliderFloat("MotionHigh##MotionAwareVRS", ref motionSettings.MotionHigh, 0.0f, 30.0f);

                        ImGui.SeparatorText("Luminance Variance");

                        ImGui.SliderFloat("LumaVarianceLow##MotionAwareVRS", ref motionSettings.LumaVarianceLow, 0.0f, 0.01f, "%.6f");
                        ImGui.SliderFloat("LumaVarianceHigh##MotionAwareVRS", ref motionSettings.LumaVarianceHigh, 0.0f, 0.05f, "%.6f");

                        ImGui.SeparatorText("Depth Protection");

                        ImGui.SliderFloat("DepthRangeThreshold##MotionAwareVRS", ref motionSettings.DepthRangeThreshold, 0.0f, 0.1f, "%.5f");

                        ImGui.SeparatorText("Foveated Radius");

                        ImGui.SliderFloat("InnerRadius##MotionAwareVRS", ref motionSettings.InnerRadius, 0.0f, 1.0f);
                        ImGui.SliderFloat("MiddleRadius##MotionAwareVRS", ref motionSettings.MiddleRadius, 0.0f, 1.0f);

                        ImGui.SeparatorText("Allowed Coarse Rates");

                        bool allow2x2 = motionSettings.Allow2x2 != 0;
                        if (ImGui.Checkbox("Allow2x2##MotionAwareVRS", ref allow2x2))
                        {
                            motionSettings.Allow2x2 = allow2x2 ? 1 : 0;
                        }

                        bool allow4x2 = motionSettings.Allow4x2 != 0;
                        if (ImGui.Checkbox("Allow4x2##MotionAwareVRS", ref allow4x2))
                        {
                            motionSettings.Allow4x2 = allow4x2 ? 1 : 0;
                        }

                        bool allow4x4 = motionSettings.Allow4x4 != 0;
                        if (ImGui.Checkbox("Allow4x4##MotionAwareVRS", ref allow4x4))
                        {
                            motionSettings.Allow4x4 = allow4x4 ? 1 : 0;
                        }

                        ImGui.SeparatorText("Bias");

                        ImGui.SliderFloat("MotionBias##MotionAwareVRS", ref motionSettings.MotionBias, 0.0f, 4.0f);
                        ImGui.SliderFloat("VarianceBias##MotionAwareVRS", ref motionSettings.VarianceBias, 0.0f, 4.0f);

                        app.RasterizerPipeline.MotionAwareVRS.Settings = motionSettings;
                ...
                if (ImGui.CollapsingHeader("MotionBlur"))
                {
                    ...
                    if (app.RasterizerPipeline.IsMotionBlur)
                    {
                        ...
                        ImGui.SliderFloat("ShutterScale", ref app.RasterizerPipeline.MotionBlur.Settings.ShutterScale, 0.0f, 2.0f);
                        ImGui.SliderFloat("MinDeltaScale", ref app.RasterizerPipeline.MotionBlur.Settings.MinDeltaScale, 0.1f, 2.0f);
                        ImGui.SliderFloat("MaxDeltaScale", ref app.RasterizerPipeline.MotionBlur.Settings.MaxDeltaScale, 0.5f, 4.0f);
                    }
                }
            }
