...
class RasterPipeline : IDisposable
{
    ...
    public enum ActiveVrsSource : int
    {
        Lighting,
        MotionAware,
    }
    ...
    public readonly MotionAwareVRSClassifier MotionAwareVRS;
    ...
    public ActiveVrsSource ActiveVariableRateShadingSource = ActiveVrsSource.Lighting;
    ...
    public RasterPipeline(Vector2i renderSize, Vector2i presentationSize)
    {
        ...
        MotionAwareVRS = new MotionAwareVRSClassifier(renderSize, new MotionAwareVRSClassifier.GpuSettings());
    ...
    public void Render(ModelManager modelManager, LightManager lightManager, Camera camera, float dT, Vector2 mousePos, Vector2 windowSize, bool isScopeMode)
    {
        ...
        }, new BBG.Rendering.GraphicsPipelineState()
        {
            ...
            VariableRateShading = GetCurrentVRSData(),
        }, () =>
        {
        ...
        bool lightingDebug = LightingVRS.Settings.DebugValue != LightingShadingRateClassifier.DebugMode.None;
        bool motionAwareDebug = MotionAwareVRS.Settings.DebugValue != MotionAwareVRSClassifier.DebugMode.None;

        if (IsVariableRateShading || lightingDebug || motionAwareDebug)
        {
            bool needsFrequencyMap =
            VariableRateShadingMode == VrsMode.FrequencyMap ||
            ( VariableRateShadingMode == VrsMode.Original && LightingVRS.Settings.UseFrequencyMapFusion == 1) ||
            LightingVRS.Settings.DebugValue == LightingShadingRateClassifier.DebugMode.FrequencyMap;

            if (needsFrequencyMap)
            {
                FrequencyVRS.Compute(
                    NormalTexture,
                    FrequencyEdgeThreshold,
                    FrequencyHighRateRatio,
                    FrequencyMedRateRatio
                );
            }

            {
                var lightingSettings = LightingVRS.Settings;
                lightingSettings.VrsMode = (int)VariableRateShadingMode;

                if (mousePos.X >= 0)
                {
                    Vector2 normalizedMouse = mousePos / windowSize;
                    normalizedMouse.Y = 1.0f - normalizedMouse.Y;
                    lightingSettings.MousePos = normalizedMouse;
                }

                lightingSettings.IsFoveated = isScopeMode ? 2 : (IsFoveated ? 1 : 0);

                LightingVRS.Settings = lightingSettings;
                LightingVRS.Compute(beforeTAATexture, FrequencyVRS.Result);
            }

            {
                var motionSettings = MotionAwareVRS.Settings;

                if (mousePos.X >= 0)
                {
                    Vector2 normalizedMouse = mousePos / windowSize;
                    normalizedMouse.Y = 1.0f - normalizedMouse.Y;
                    motionSettings.MousePos = normalizedMouse;
                }

                motionSettings.IsFoveated = isScopeMode ? 2 : (IsFoveated ? 1 : 0);

                MotionAwareVRS.Settings = motionSettings;
                MotionAwareVRS.Compute(beforeTAATexture);
            }
        }

        if (IsMotionBlur)
        {
            MotionBlur.Settings.DeltaTime = dT;
            MotionBlur.Settings.ReferenceDeltaTime = 1.0f / 60.0f;
    ...
    private BBG.Rendering.VariableRateShadingNV GetCurrentVRSData()
    {
        if (ActiveVariableRateShadingSource == ActiveVrsSource.MotionAware)
        {
            return MotionAwareVRS.GetRenderData();
        }
    ...
    public void SetSize(Vector2i renderSize, Vector2i presentationSize)
    {
        ...
        MotionAwareVRS.SetSize(renderSize);
